...
namespace Lab1.Shapes
{
    ...
    {
        // Public key (loaded from XML)
        private static string PublicKeyXml = "";

        // Data from .sig file
        public class PluginSignature
        {
            public string Signature { get; set; } = "";      // Signature in Base64
            public DateTime SigningTime { get; set; }        // Signing time
            public DateTime Expiration { get; set; }         // Expiration date
        }
        ...
        public static bool VerifyPlugin(string dllPath, out string error)
        {
            ...
            try
            {
                ...
                {
                    error = $"Plugin expired (valid until {sigInfo.Expiration})";
                    return false;
                }
                ...
                Console.WriteLine($"[DEBUG] Expiration date: {sigInfo.Expiration}");

                // Compare DLL hash with signature
